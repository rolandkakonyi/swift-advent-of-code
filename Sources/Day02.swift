...
struct Day02: AdventDay {
        ...
        data
            ...
            .map { line -> Game in
                ...
                let sets = setsStrings.map { setString in
                    let (red, green, blue) = setString.split(separator: ",").map { $0.trimmingCharacters(in: .whitespacesAndNewlines)}
                        .reduce((red: 0, green: 0, blue: 0)) { partialResult, currentValue in
                            var (red, green, blue) = partialResult
                            var buffer = ""
                            currentValue.forEach { character in
    ...
    func part2() -> Any {
        games
            .map { game -> Int in
                let (red, green, blue) = game.sets.reduce((red: 0, green: 0, blue: 0)) { partialResult, set in
                    (max(partialResult.red, set.red), max(partialResult.green, set.green), max(partialResult.blue, set.blue))
                }
                return red * green * blue
            }
            .reduce(0, +)
